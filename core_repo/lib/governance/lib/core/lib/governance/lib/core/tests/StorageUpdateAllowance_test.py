...
from Tag_test import tag0, tag2, tag3
...
@pytest.mark.parametrize('owner', [address0, address2, address3])
@pytest.mark.parametrize('tag', [tag0, tag2, tag3])
@pytest.mark.parametrize('amount', [balance0, balance2, balance3, balance4, value0, value2, value3])
def test_updateAllowance(wrapper, currentIsOperator, currentAllowance, owner, tag, amount, request, worker_id):
    ...
    if not(currentIsOperator) and (amount > currentAllowance):
        with brownie.reverts('InsufficientPermission: ' + accounts[0].address.lower() + ', ' + str(tag)):
            tx = wrapper._updateAllowance(currentIsOperator, currentAllowance, owner, tag, amount)
    else:
        tx = wrapper._updateAllowance(currentIsOperator, currentAllowance, owner, tag, amount)
        newAllowance = tx.return_value
