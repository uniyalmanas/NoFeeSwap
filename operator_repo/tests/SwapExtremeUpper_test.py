...
def test_swapExtreme(deployment, n, request, worker_id):
    ...
    spacing = upper - lower
    curve = [point - upper + (2 ** 64) - spacing - 2 for point in curve]
    target = target - upper + (2 ** 64) - spacing - 2
    lower = (2 ** 64) - 2 * spacing - 2
    upper = (2 ** 64) - spacing - 2
    ...
    assert token0.balanceOf(nofeeswap) <= 100
    assert token1.balanceOf(nofeeswap) <= 100