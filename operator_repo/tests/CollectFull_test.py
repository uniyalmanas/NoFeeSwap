...
from Nofee import logTest, _hookData_, _msgSender_, _hookDataByteCount_, address0, Pool, donateSequence, collectSequence, mintSequence, burnSequence, swapSequence, keccak, toInt, twosComplementInt8, encodeKernelCompact, encodeCurve, dataGeneration, checkPool, getPoolId
...
def deployment(fn_isolation):
    ...
    protocolGrowthPortion = (1 << 47)
    poolGrowthPortion = (1 << 47)
...
def test_swapCollectFullProtocolFirst(deployment, n, request, worker_id):
    ...
    data = collectSequence(token0, token1, tag0, tag1, owner, amount0, amount1, deadline)
...
@pytest.mark.parametrize('n', range(len(swaps['kernel'])))
def test_swapCollectFullPoolFirst(deployment, n, request, worker_id):
    logTest(request, worker_id)
