...
def test_swapInRange(deployment, n, request, worker_id):
    ...
    amountSpecified = 1 - (1 << 127)
    limit = target - (1 << 63) + (logOffset * (1 << 59))
    zeroForOne = 1 if target < current else 0
    ...
    amountSpecified = - ((token1.balanceOf(nofeeswap) // 2) if zeroForOne else (token0.balanceOf(nofeeswap) // 2))
    limit = (lower if zeroForOne else upper) - (1 << 63) + (logOffset * (1 << 59))
    zeroForOne = 2
    ...
    amountSpecified = ((token1.balanceOf(nofeeswap) // 2) if zeroForOne else (token0.balanceOf(nofeeswap) // 2))
    limit = (upper if zeroForOne else lower) - (1 << 63) + (logOffset * (1 << 59))
    zeroForOne = 2
