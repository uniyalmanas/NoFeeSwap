...
def test_modifyPoolGrowthPortionOdd(deployment, n, request, worker_id):
    ...
    event = (0x200000000000) << 208

    assert tx.events['ModifyPoolGrowthPortion']['poolId'] == poolId
    assert tx.events['ModifyPoolGrowthPortion']['caller'] == owner.address
    assert toInt(tx.events['ModifyPoolGrowthPortion']['data'].hex()) == event
    ...
    assert poolGrowthPortion == 0x200000000000
    ...
    _outgoingMaxModularInverse = pow(_outgoingMaxModularInverse, -1, 2**256)
    assert outgoingMaxModularInverse == _outgoingMaxModularInverse
    assert poolGrowthPortion == 0x400000000000
    assert pendingKernelLength == len(newKernel)
    assert maxPoolGrowthPortion == 123
    assert protocolGrowthPortion == 456

    tag0, tag1, sqrtOffset, sqrtInverseOffset, sqrtSpacing, sqrtInverseSpacing = access._readStaticParams0(nofeeswap, poolId, staticParamsStoragePointerExtension + 1)
    outgoingMax, outgoingMaxModularInverse, incomingMax, poolGrowthPortion, maxPoolGrowthPortion, protocolGrowthPortion, pendingKernelLength = access._readStaticParams1(nofeeswap, poolId, staticParamsStoragePointerExtension + 1)
    kernelArray = list(access._readKernel(nofeeswap, poolId, staticParamsStoragePointerExtension + 1))
    ...
    assert staticParamsStoragePointerExtension == (1 << 16) + 2
    assert growth == 1 << 111
    assert abs(integral0 - outgoing(curve, kernel, current, upper)) <= 2 ** 32
    assert abs(integral1 - outgoing(curve, kernel, lower, current)) <= 2 ** 32
    assert sharesTotal == shares
    ...
    _outgoingMaxModularInverse = pow(_outgoingMaxModularInverse, -1, 2**256)
    assert outgoingMaxModularInverse == _outgoingMaxModularInverse
    assert poolGrowthPortion == 0x300000000000
    assert pendingKernelLength == len(newKernel)
    assert maxPoolGrowthPortion == 123
    assert protocolGrowthPortion == 456
...
@pytest.mark.parametrize('n', range(len(initializations['kernel'])))
def test_modifyPoolGrowthPortionEven(deployment, n, request, worker_id):
    logTest(request, worker_id)
    ...
    assert abs(integral1 - outgoing(curve, kernel, lower, current)) <= 2 ** 32
    assert sharesTotal == shares
    assert staticParamsStoragePointer == (1 << 16) - 3
    assert logPriceCurrent == current
    assert tag0 == min(toInt(token0.address), toInt(token1.address))
    assert tag1 == max(toInt(token0.address), toInt(token1.address))
    assert sqrtOffset == floor((2 ** 127) * exp(Integer(logOffset) / 2))
