...
interface INofee {
    /// @notice An event that's emitted when the minter's address is changed
    ...
    function checkpoints(address account, uint32 index) external view returns (uint32 fromBlock, uint96 votes);
