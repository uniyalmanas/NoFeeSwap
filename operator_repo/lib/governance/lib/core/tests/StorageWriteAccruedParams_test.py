...
@pytest.mark.parametrize('poolId', [value0, value2, value3, value4])
@pytest.mark.parametrize('poolRatio0', [ratio0, ratio1, ratio3])
@pytest.mark.parametrize('poolRatio1', [ratio0, ratio1, ratio3])
@pytest.mark.parametrize('accrued_0', [accrued0, accrued1, accrued2, accrued4])
@pytest.mark.parametrize('accrued_1', [accrued0, accrued1, accrued2, accrued4])
def test_writeAccruedParams(wrapper, poolId, poolRatio0, poolRatio1, accrued_0, accrued_1, request, worker_id):
    logTest(request, worker_id)
    
    # Check if the storage slots dedicated to accrued parameters are written correctly.
    if accrued_0 > accruedMax:
        with brownie.reverts('AccruedGrowthPortionOverflow: ' + str(accrued_0)):
            tx = wrapper._writeAccruedParams(poolId, poolRatio0, poolRatio1, accrued_0, accrued_1)
    elif accrued_1 > accruedMax:
        with brownie.reverts('AccruedGrowthPortionOverflow: ' + str(accrued_1)):
            tx = wrapper._writeAccruedParams(poolId, poolRatio0, poolRatio1, accrued_0, accrued_1)
    else:
        tx = wrapper._writeAccruedParams(poolId, poolRatio0, poolRatio1, accrued_0, accrued_1)
        content = tx.return_value
        assert (accrued_0 >> 127) << 127 == (content % (1 << 104)) << 127
        assert (accrued_1 >> 127) << 127 == ((content >> 104) % (1 << 104)) << 127
