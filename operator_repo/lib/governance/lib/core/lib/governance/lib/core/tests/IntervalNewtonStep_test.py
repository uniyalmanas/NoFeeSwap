...
def test_getNewtonStep(
    ...
    sign, step, integral0Amended, integral1Amended = tx.return_value
    ...
    overshootMinusOrigin = (exp(- 8) / 2) * (c0 + (c1 - c0) * (qOvershoot - b0) / (b1 - b0))
    outgoingTotal = floor(X216 * outgoingTotal)
    overshootMinusOrigin = floor(X216 * overshootMinusOrigin)
    ...
    overshootMinusTarget = (exp(- 8) / 2) * (c0 + (c1 - c0) * (qOvershoot - b0) / (b1 - b0))
    outgoingForward = floor(X216 * outgoingForward)
    overshootMinusTarget = floor(X216 * overshootMinusTarget)
    ...
    if zeroForOne:
        integral1AmendedPrime = originToTarget + \
            floor(2 * exp(- qOrigin / 2) * overshootMinusOrigin) - \
            floor(2 * exp(- qTarget / 2) * overshootMinusTarget)
        
        integral0AmendedPrime = \
            floor(2 * exp(+ qOvershoot / 2) * (overshootMinusOrigin - overshootMinusTarget))
    else:
        integral1AmendedPrime = originToTarget + \
            floor(2 * exp(+ qOrigin / 2) * overshootMinusOrigin) - \
            floor(2 * exp(+ qTarget / 2) * overshootMinusTarget)
    ...
    if zeroForOne:
        mismatch = (
            integral0Incremented * _integral1Amended
        ) - (
            integral1Incremented * _integral0Amended
        )
    ...
    _step = ((1 << 38) * mismatch) / (mismatchPrime / (1 << 22))
