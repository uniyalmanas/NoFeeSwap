...
from Nofee import logTest, encodeCurve
...
def test_readCurve(wrapper, poolId, qCurrent, curve, offset, length, request, worker_id):
    logTest(request, worker_id)
    
    # Check if the curve sequence are read correctly.
    curveArray = ([curve] * length) + [((curve << (64 * (offset + 1))) % (2 ** 256)) + (qCurrent << (64 * offset))]
    tx = wrapper._readCurve(poolId, qCurrent, curveArray)
    curveMembers, curveLength = tx.return_value
    assert curveLength == 4 * length + 4 - offset
    assert encodeCurve(curveMembers) == curveArray