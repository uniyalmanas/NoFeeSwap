...
contract SearchIncomingTest {
  function searchIncomingTarget_test(
    ...
    setIntegralLimit(get_an_integral(
      1 + integral_seed % uint216(
        uint256(X216.unwrap(_total0_.incoming(_begin_, _target_))) - 1
      )
    ));
    setZeroForOne(zeroForOne_seed);

    (, X216 incoming) = searchIncomingTarget();
    X216 incomingLimit = 
      getIntegralLimit() - _incomingCurrentToTarget_.integral();

    X59 x = _target_.log();
    assert(incoming <= incomingLimit);
    if (x != _total1_.log()) {
      x = getZeroForOne() ? x - epsilonX59 : x + epsilonX59;
      _target_.storePrice(x);
      assert(incomingLimit < _total0_.incoming(_begin_, _target_));
    }
  }
}