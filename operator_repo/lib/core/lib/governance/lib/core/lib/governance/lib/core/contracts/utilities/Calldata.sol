...
/// @notice Reads inputs of the external function 'quoteModifyPosition' and
/// places each in the appropriate memory location.
function readQuoteModifyPositionInput() pure {
  // Calldata layout for 'quoteModifyPosition' is as follows:
  //
  // '0x00': function selector
  // '0x04': 'poolId'
  // '0x24': 'logPriceMin'
  ...
  setLogPriceMin(logPriceMin);
  X59 logPriceMinOffsetted = logPriceMin - shift;
  require(
    logPriceMinOffsetted < thirtyTwoX59,
    LogPriceOutOfRange(logPriceMin)
  );
  setLogPriceMinOffsetted(logPriceMinOffsetted);
  ...
  setLogPriceMax(logPriceMax);
  X59 logPriceMaxOffsetted = logPriceMax - shift;
  require(
    logPriceMaxOffsetted < thirtyTwoX59,
    LogPriceOutOfRange(logPriceMax)
  );
  setLogPriceMaxOffsetted(logPriceMaxOffsetted);
  ...
  assembly {
    // 32 bytes are reserved for the first slot of the curve sequence and no
    // member of kernel is loaded.
    freeMemoryPointer := add(_endOfStaticParams_, 32)
  }
  setFreeMemoryPointer(freeMemoryPointer);
}

/// @notice Reads inputs of the external function 'quoteDonate' and
/// places each in the appropriate memory location.
function readQuoteDonateInput() pure {
  // Calldata layout for 'donate' is as follows:
  //
  // '0x00': function selector
  // '0x04': 'poolId'
  ...
  uint256 freeMemoryPointer;
  assembly {
    // 32 bytes are reserved for the first slot of curve and no member of
