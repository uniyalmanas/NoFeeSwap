...
// efb0dec66562c494ea54d67adfe0939d759c275e/src/Exttload.sol
pragma solidity ^0.8.28;

import {ITransientAccess} from "./interfaces/ITransientAccess.sol";

contract TransientAccess is ITransientAccess {
  /// @inheritdoc ITransientAccess
  function transientAccess(
    bytes32 slot
  ) external view override returns (bytes32) {
    assembly ("memory-safe") {
      // The input slot is read from transient storage and stored in memory 
      // scratch space to be returned.
      mstore(0, tload(slot))
      return(0, 0x20)
    }
  }
  ...
  function transientAccess(
    ...
    assembly ("memory-safe") {
      ...
      for {} 1 {} {
        // The current slot is read from transient storage and placed in 
        // memory.
  ...
  function transientAccess(
    ...
    assembly ("memory-safe") {
      ...
      // A loop over the number of slots to be read.
      for {} 1 {} {
        // The slot given by the 'calldataPointer' is read from transient 
